...
- **Contacts**: People in your network; collaborations linked to releases and library songs (with optional context notes).
...
The assistant has **mutation tools** (update/delete on releases, library songs, feedback links, contacts). Each call **queues a proposal**; the user must tap **Approve** or **Reject** in the assistant UI before anything is written. Until then, the database is unchanged.
...
- **Contacts**: **create** new contacts; **update** existing rows (by id from list); delete contact (and collaboration rows via FK cascade where applicable).
